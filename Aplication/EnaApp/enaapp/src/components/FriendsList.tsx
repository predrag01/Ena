...
const FrindsList = (props: {userId: number, chat: boolean, setUser?: (user: User) => void, refetchFriends:boolean, connection:signalR.HubConnection|null, gameId: number}) => {
    ...
    return (
        <div className="friend-list">
            ...
            {frinedsList.map((friend, id) => {
                return (props.chat ? <ChatItem result={friend} key={id} setUser={props.setUser || (() => {})}/> : <Friend result={friend} key={id} connection={props.connection} gameId={props.gameId}/>)
            })}
        </div>
    );
};
